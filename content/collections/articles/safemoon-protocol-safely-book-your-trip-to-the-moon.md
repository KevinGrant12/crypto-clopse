---
id: b4683e62-5677-4b45-8c55-eefa0c1475b2
blueprint: articles
title: 'SafeMoon Crypto: Secure Your Rocket Ride to the Moon'
key_points:
  - 'What is SafeMoon Protocol?'
  - 'Where and how to buy SafeMoon'
  - 'What''s on the horizon?'
category:
  - crypto-spotlight
tags:
  - featured
crypto_brands:
  - 9110f676-be85-44db-bfad-6d4251222c43
updated_by: b0661e95-7c22-4bb7-a365-8c56bd87bf1d
updated_at: 1627942949
author: 03b19c0a-ef4b-405a-8a5d-2e1e03932f33
featured_image: assets/articles/article-safemoon_safemoon-protocol.jpg
preview: 'SafeMoon is a fairly new cryptocurrency that rewards long-term investment by redistributing a percentage of each transaction to it''s holders.'
Category:
  - crypto-spotlight
content:
  -
    type: heading
    attrs:
      level: 3
    content:
      -
        type: text
        text: 'What is SafeMoon Crypto?'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'The SafeMoon cryptocurrency is a fairly new coin rewards long-term investment by redistributing a percentage of each transaction to it''s holders. Anytime someone buys or sells there is a 10% fee. 5% goes to all holders, while the other 5% is permanently "burned." Personally, I have accumulated around 30 million coins in under a month just from being a holder. That is just the bird''s eye view. Let''s zoom in and see what fuels this rocket.'
  -
    type: blockquote
    content:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The SafeMoon cryptocurrency is a community driven, fair launched DeFi Token. Three simple functions occur during each trade: Reflection, LP Acquisition, & Burn.'
  -
    type: heading
    attrs:
      level: 4
    content:
      -
        type: text
        text: 'Community Driven'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Since its initial launch on PancakeSwap in early March of 2021, the SafeMoon community has grown to a wapping 285,000 holders with hundreds of more followers on social avenues like '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://twitter.com/safemoon'
              rel: null
              target: _blank
              title: null
        text: Twitter
      -
        type: text
        text: ' and '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://discord.com/'
              rel: null
              target: _blank
              title: null
        text: Discord
      -
        type: text
        text: '. SafeMoon is registered as an LLC and the team has been highly active within the growing community including holding many AMA''s ( ask me anything ) and rapidly updating their website with statistics, a roadmap, merchandise, educational features, and much more. Ok, sounds like a white-hot start, but will the value of the coin grow beyond just market speculation?'
  -
    type: heading
    attrs:
      level: 4
    content:
      -
        type: text
        text: 'Deflationary Coin'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'The SafeMoon cryptocurrency is a deflationary coin meaning the number of coins in existence will decrease over time. At the time of writing this article, April 05 of 2021, SafeMoon has burned roughly 400 trillion coins and swiftly counting. This means that the intrinsic value of the coin should steadily grow over time as the availability decreases.'
  -
    type: heading
    attrs:
      level: 4
    content:
      -
        type: text
        text: 'The Core Team'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'In order to build a foundation of trust, the developer team burned all of their tokens and participated in the fair launch with everyone else. Even some high-ranking members, such as CTO Thomas "Papa" Smith, refuse to hold SafeMoon in order to avoid any conflicts of interest. In just the first month, I witnessed the SafeMoon team grow to 50+ individuals with an eclectic skillset. Let''s take a look at the leadership:'
  -
    type: table
    content:
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    marks:
                      -
                        type: bold
                    text: Name
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    marks:
                      -
                        type: bold
                    text: Position
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    marks:
                      -
                        type: bold
                    text: Quote
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'John Karony - "Capt Hodl"'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: CEO
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: '"Imminent"'
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Thomas Smith - Papa'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: CTO
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: '"My hat''s around somewhere"'
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'SafeMoon Dev'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: Mystery
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Silence is golden'
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Henry (Hank) Wyatt'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Director of SE'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: '"It''s a cat, ok?"'
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Jack Haines'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: COO
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Jacob Smith - "OG KingFox"'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Website Developer'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
      -
        type: table_row
        content:
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Charles Karony'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
                content:
                  -
                    type: text
                    text: 'Executive Assistant'
          -
            type: table_cell
            attrs:
              colspan: 1
              rowspan: 1
              colwidth: null
              background: null
            content:
              -
                type: paragraph
  -
    type: heading
    attrs:
      level: 3
    content:
      -
        type: text
        text: 'Where can I buy SafeMoon?'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'The earliest of birds were stuck traversing what would be considered a small labyrinth of transfers and conversions in order to stake their claim of some SafeMoon crypto. As an advanced computer user, my first acquisition attempt ( paranoia and all ) took about an hour to complete using Binance.us to buy BNB coin, Trust Wallet to hold and then move '
      -
        type: text
        marks:
          -
            type: italic
        text: that
      -
        type: text
        text: ' BNB over to the BNB Smartchain, and finally PancakeSwap to exchange BNB for SafeMoon. In retrospect, it''s really not complicated and I can now complete the process in about 5 minutes.'
      -
        type: hard_break
      -
        type: hard_break
      -
        type: text
        text: 'The good news, however, is that John Karony ( CEO ) and the team have been firing on all cylinders while rapidly working to expand their reach into more accessible marketplaces. In fact, as of today, SafeMoon has officially been listed on '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://www.bitmart.com/en'
              rel: null
              target: _blank
              title: null
        text: BitMart
      -
        type: text
        text: ' and '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://medium.com/whitebit/safemoon-listed-db7fe8c0837e'
              rel: null
              target: _blank
              title: null
        text: WhiteBIT
      -
        type: text
        text: '. That said, there are still several tiers of accessibility to climb in order to reach the average investor. This remains true with crypto general, though. So, if you are reading this article, you may just have reached the ship in-time for some stellar gains!'
  -
    type: heading
    attrs:
      level: 3
    content:
      -
        type: text
        text: 'What''s on the horizon?'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'The SafeMoon crypto road map is broken down quarterly. April marks the date in which we have officially departed the Q1 operations. The rocket is burning at full-force as we approach the atmosphere and prepare for our meteoric ascent to the moon. The team has successfully doubled in size, is now listed on 3 marketplaces, has some cool '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://shop.safemoon.net/'
              rel: null
              target: _blank
              title: null
        text: merch
      -
        type: text
        text: ' available to the public.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Q2 looks extremely ambitious with promises of NFT''s, video game integration, a SafeMoon charity drive, integration into African markets, increasing the team by another 35%, and opening a UK/Ireland office. In fact, I believe John is already in the UK getting shit done. If Q1 was any indication at all for this team''s ability to exceed their goals, then we are in for some serious g-force throughout the rest of 2021.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'So strap on your astro boots, pack your barf bag, and get ready to lose your lunch while this rocket booms into the crypto sphere. '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'https://safemoon.net/'
              rel: null
              target: _blank
              title: null
        text: SafeMoon
      -
        type: text
        text: ' crypto is here to propel you '
      -
        type: text
        marks:
          -
            type: bold
        text: 'Safely to the Moon!'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Want to learn more about the SafeMoon crypto? Checkout this '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'statamic://entry::c9beb457-0a52-43e8-83dd-32521f221d7b'
              rel: null
              target: _blank
              title: null
        text: article
      -
        type: text
        text: ' about the resilience of SafeMoon and the the core team. '
  -
    type: paragraph
    content:
      -
        type: text
        marks:
          -
            type: bold
        text: 'Something Extra'
  -
    type: blockquote
    content:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Love DogeCoin? So does the SafeMoon team. Prepare yourself for the "imminent" SafeMoon/DogeCoin trading pair on '
          -
            type: text
            marks:
              -
                type: link
                attrs:
                  href: 'https://www.bibipom.com/'
                  rel: null
                  target: _blank
                  title: null
            text: Bibipom
          -
            type: text
            text: .
---
